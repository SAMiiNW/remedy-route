...
def test_duplicate_origins_and_forged_decision_rejected(direct_vm,direct_deploy,direct_alice,direct_bob):
 ...
 r.open_route('rr-1','0x'+direct_bob.hex(),ISSUE,TRACKS,A,B,3600);seed(direct_vm);out=r._decide(r.routes['RR-1']);assert direct_vm.run_validator(leader_result=out) is True;bad=dict(out);bad['digests']=['forged','forged'];assert direct_vm.run_validator(leader_result=bad) is False
